...
def run_cora(aggr1, aggr2):
    ...
    agg1 = MaxAggregator(features, cuda=False) if aggr1 == "max" else MeanAggregator(features, cuda=False)
    enc1 = Encoder(features, NUM_FEATS_CORA, EMBED_DIM, adj_lists, agg1, NUM_SAMPLES_CORA_LAYER1, gcn=True, cuda=False)
    agg2 = MaxAggregator(lambda nodes: enc1(nodes).t(), cuda=False) if aggr2 == "max" else MeanAggregator(lambda nodes: enc1(nodes).t(), cuda=False)
